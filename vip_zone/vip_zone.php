...
?>
		  ...
		  if($parameter =="catart") {
			  ...
			  // A FAIRE :verifier si il y a bien une page statique en acceuil, sinon ç'est une mise en orbite du visiteur
				if (!isset($_SESSION['profil']) && ($plxMotor->mode !== 'static' )) {
					header("Location: ".PLX_ROOT."core/admin/auth.php?page=".$_SERVER['REQUEST_URI']);
				}
		}		
<?php
	    ...
	    echo self::BEGIN_CODE;
?>
		 if (isset($_SESSION['profil'])) { array_push($menus, '<li class="static menu noactive"  id="VIP_logout"><a href="'.PLX_ROOT.'core/admin/auth.php?d=1&logout=1"  title="exit">X</a></li>');}
<?php
            echo self::END_CODE;
        }			
