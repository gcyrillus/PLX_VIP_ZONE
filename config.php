		...
		if ((file_exists(PLX_ROOT.PLX_CONFIG_PATH.'users.xml')) && (($open = fopen(PLX_PLUGINS.'/vip_zone/username.csv', 'r')) !== FALSE) && (!isset($_GET['upmsg']))) {
			...
			foreach($array as $i => $line){ 
				...
				if($i >0) { // on passe la premiere ligne ou sont  stockées les entêtes de colonnes.
					$nbRecords++; // on compte les enregistrements qui sont ajoutés.
					
					//test si données présentes
					if((!isset($line[0])) or (!isset($line[1])) or (!isset($line[2])) or (!isset($line[3]))) {
						...
						if (in_array( $line[0], $loginUser) && $line[0] !=''  && $line[1] !='' || in_array( $line[1], $nameUser)) {// verif doublon 
						...
						if (in_array( $line[1], $nameUser) && $line[1] !='') {// verif doublon
				...
				if(isset($line[0])  && trim($line[0]) != 'LOGIN' && isset($line[1]) && isset($line[2]) && isset($line[3])) {
